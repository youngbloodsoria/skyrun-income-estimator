...
4. In Supabase Authentication:
   ...
   - Update the email template so it includes `{{ .Token }}`. Owners and approved employees enter the code generated by the shared Supabase project.
...
Admins can add employees from `/admin`. New employees must use a `@skyrun.com` address and sign in using an emailed verification code.
